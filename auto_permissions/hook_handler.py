...
def run_hook() -> None:
    try:
        ...
        if not raw_input.strip():
            print(json.dumps({"decision": "force_ask", "reason": "No input received on hook stdin."}), flush=True)
            return
        ...
        try:
            ...
            context = {
                "workspace_paths": data.get("workspacePaths", []) if isinstance(data, dict) else [],
                "artifact_dir": data.get("artifactDirectoryPath", "") if isinstance(data, dict) else "",
                "conversation_id": (data.get("conversationId") or "") if isinstance(data, dict) else "",
                "transcript_path": data.get("transcriptPath", "") if isinstance(data, dict) else "",
            }
            ...
            result = {
                "decision": fallback,
                "reason": f"Hook evaluation error ({str(e)}). Deferring to '{fallback}'.",
                "source": "ERROR",
            }
            latency_ms = 0.0
        ...
        debug_log = config.get("debug_log") if isinstance(config, dict) else None
        if debug_log:
            try:
        ...
        print(json.dumps({"decision": "force_ask", "reason": f"Fatal hook error ({str(e)}). Deferring to user."}), flush=True)
